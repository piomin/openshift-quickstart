...
In the `src/main/resources` directory create folder `avro`. Then, create a file `callmeevent.avsc` with the following content:
```json
{
  "type":"record",
  "name":"CallmeEvent",
...
@Bean
@Primary
public SchemaRegistryClient schemaRegistryClient(RestTemplate restTemplate, SchemaRegistryClientProperties properties) {
...
```
Go to the Apicurio UI. Find your application on the list of artifacts. Click it. Find your schema. Go to the details. See a new version. Switch to the `Content` tab. \
Back to `Info` tab. In the section `Content` enable `Compatibility Rule`. Choose `Forward` option. \
Then open your schema definition in `src/main/resources/avro/callmeevent.avsc` and add a new field:
```json
{
  "type":"record",
  "name":"CallmeEvent",
...
```
Rebuild your application with `mvn clean package`. Modify `CallmeEvent` in the application main class with a new field. \
Push your application to the OpenShift cluster and verify logs. See a current content. Then see a list of versions. \
Now let's provide some non-compliant changes in the `callmeevent.avsc` schema, e.g. change the name of the field from `message` into `content` and remove a single field:
```json
{
  "type":"record",
  "name":"CallmeEvent",
...
Then you can change a compatibility rule to `Backward`. Remove only a field `priority` in comparison to the `2` version. \
Do some compliant changes like adding a new field into `callmeevent.avsc` without removing anything. Replay the procedure as before. \
Then go to the Apicurio Schema UI once again. 
